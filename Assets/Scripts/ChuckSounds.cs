...
public class ChuckSounds : MonoBehaviour
{
    ...
    Dictionary<string, string> chuckStore = new Dictionary<string, string>();
    string myChucKAudio;
    bool checkChucKSnd = false;
    ...
    private void Start()
    {
        ...
        checkChucKSnd = CheckChucK();

        chuckStore.Add("Animal", @"
		        TriOsc Osc => dac;
                1 => Osc.gain;
    ...
    public void PlayChuck()
    {
        if(checkChucKSnd)
        {
            chuckSnd.RunCode(myChucKAudio);
        }
        else
        {
            Debug.Log("I have no sound ");
        }
    }

    public void FindMyAudio(string iAm)
    {
        myChucKAudio = chuckStore[iAm];
    }

    private bool CheckChucK()
    {
        if (chuckSnd)
        {
            return true;
